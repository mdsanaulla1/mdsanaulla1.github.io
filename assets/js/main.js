...
$(function(){
    ...
    if ($('.activity-slider').length) {
        var activity = $('.activity-slider').owlCarousel({
            items: 1,
            margin: 30,
            stagePadding: 0,
            smartSpeed: 450,
        ...
        $('.activity-nav .next').on('click', function() {
            activity.trigger('next.owl.carousel');
        })
        $('.activity-nav .prev').on('click', function() {
            activity.trigger('prev.owl.carousel', [300]);
        })
        ...
        $('#activity-slide-count').html("<span class='left'>"+item+"</span> / "+items)
        }
    }
